...
def asset_blocks() -> str:
    css_line = "    " + LT + "link rel=" + QUOTE + "stylesheet" + QUOTE + " href=" + QUOTE + "{{ url_for('static', filename='css/documents_to_sign.css') }}" + QUOTE + GT
    js_line = "    " + LT + "script src=" + QUOTE + "{{ url_for('static', filename='js/documents_to_sign.js') }}" + QUOTE + GT + LT + "/script" + GT
    return "\n".join([
        "{% block extra_css %}",
        css_line,
        "{% endblock %}",
