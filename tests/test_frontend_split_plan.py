...
def test_frontend_split_plan_tracks_extracted_documents_assets():
    plan = Path("docs/archive/FRONTEND_SPLIT_PLAN.md").read_text(encoding="utf-8")
    template = Path("templates/documents_to_sign.html").read_text(encoding="utf-8")

    assert "static/css/documents_to_sign.css" in plan
    assert "static/js/documents_to_sign.js" in plan
