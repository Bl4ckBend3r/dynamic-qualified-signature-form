(function () {
  ...
  function initializePopupButtons() {
    document
      .querySelectorAll(
        "[data-agreement-builder-popup], [data-document-builder-popup]",
      )
      .forEach((button) => {
        button.addEventListener("click", () => {
          const formId = String(button.dataset.formId || "");
          const documentType = String(
            button.dataset.documentType || "agreement",
          );
          const identity = `${documentType}:${formId}`;
          const builderUrl = button.dataset.builderUrl;
          const launcher = button.closest(
            '[data-source-panel="builder"], .agreement-builder-launcher, .document-builder-launcher',
          );
          const error = launcher?.querySelector(
            "[data-agreement-builder-popup-error], [data-document-builder-popup-error]",
          );
          ...
          if (!agreementBuilderWindow) {
            if (error) {
              error.textContent =
                "Przeglądarka zablokowała otwarcie kreatora w nowym oknie. Zezwól na wyskakujące okna dla tej strony i spróbuj ponownie.";
              error.hidden = false;
            }
            return;
          }
    ...
    window.addEventListener("message", (event) => {
      if (
        ...
        !["agreement-builder-saved", "document-builder-saved"].includes(
          event.data?.type,
        )
      )
        return;
      const button = event.data.documentType
        ? document.querySelector(
            `[data-document-builder-popup][data-document-type="${String(event.data.documentType)}"][data-form-id="${String(event.data.formId)}"]`,
          )
        : document.querySelector(
            `[data-agreement-builder-popup][data-form-id="${String(event.data.formId)}"]`,
          );
      const scope = button?.closest("[data-document-source-scope]");
      const status = scope?.querySelector(
        "[data-agreement-builder-parent-status], [data-document-builder-parent-status]",
      );
  ...
  function initializeSourceSwitching() {
    ["contract", "declaration"].forEach((prefix) => {
      const radios = [
        ...document.querySelectorAll(`input[name="${prefix}_template_source"]`),
      ];
      if (!radios.length) return;
      const scope =
        radios[0].closest("[data-document-source-scope]") || document;
      const sync = () => {
        const source =
          radios.find((radio) => radio.checked)?.value || "builder";
        scope.querySelectorAll("[data-source-panel]").forEach((panel) => {
          panel.hidden = panel.dataset.sourcePanel !== source;
        });
        scope
          .querySelectorAll("[data-document-variable-catalog]")
          .forEach((catalog) => {
            catalog.hidden = source === "builder";
          });
        scope
          .querySelectorAll("[data-document-source-option]")
          .forEach((option) => {
    ...
    document
      ...
      .forEach((catalog) => {
        ...
        const status = catalog.querySelector(
          "[data-document-variable-copy-status]",
        );
        ...
        const filter = () => {
          ...
          let visibleCount = 0;
          catalog
            .querySelectorAll("[data-document-variable-card]")
            .forEach((item) => {
              const visible =
                !query ||
                String(item.dataset.variableSearchText || "")
                  .toLocaleLowerCase("pl-PL")
                  .includes(query);
              item.hidden = !visible;
          ...
          catalog
            .querySelectorAll("[data-document-variable-category]")
            .forEach((category) => {
              category.hidden = !category.querySelector(
                "[data-document-variable-card]:not([hidden])",
              );
            });
        ...
        catalog.addEventListener("click", async (event) => {
          ...
          try {
            ...
            if (status)
              status.textContent =
                "Nie udało się skopiować zmiennej. Zaznacz ją ręcznie.";
          }
        });
        filter();
      });
  ...
  function initializeBuilder(root) {
    ...
    const variables = parseJson(
      root.querySelector("[data-agreement-builder-variables]"),
      [],
    );
    const submissions = parseJson(
      root.querySelector("[data-agreement-builder-submissions]"),
      [],
    );
    const criteriaVariables = variables.filter(
    ...
    const previewPage = root.querySelector(
      "[data-agreement-builder-preview-page]",
    );
    const previewCanvas = root.querySelector("[data-builder-preview-canvas]");
    const previewViewport = root.querySelector(
      "[data-builder-preview-viewport]",
    );
    const previewState = root.querySelector(
      "[data-agreement-builder-preview-state]",
    );
    ...
    const variablesPane = root.querySelector(
      "[data-agreement-builder-variable-panel]",
    );
    const previewData = root.querySelector("[data-builder-preview-data]");
    const previewTraining = root.querySelector(
      "[data-builder-preview-training]",
    );
    const previewTrainingField = root.querySelector(
      "[data-builder-preview-training-field]",
    );
    ...
    const localKey =
      root.dataset.storageKey ||
      `document-builder:${documentType}:form:${formId}:v${BUILDER_SCHEMA_VERSION}`;
    const legacyLocalKey =
      root.dataset.legacyStorageKey ||
      `document-builder-${documentType}-draft-${formId}`;
    ...
    const embeddedBackendDocument = parseJson(initialNode, {
      version: 1,
    ...
    function debugEvent(eventName, details = {}) {
      ...
      console.debug(`[document-builder] ${eventName}`, {
        documentType,
        formId,
        ...details,
      });
    ...
    try {
      ...
      if (["split", "editor", "preview"].includes(savedUi?.viewMode))
        builderState.viewMode = savedUi.viewMode;
      if (typeof savedUi?.variablesVisible === "boolean")
        builderState.variablesVisible = savedUi.variablesVisible;
      if (
        ["balanced", "editor-wide", "preview-wide"].includes(
          savedUi?.workspaceSizeMode,
        )
      )
        builderState.workspaceSizeMode = savedUi.workspaceSizeMode;
      if (Number.isFinite(savedUi?.previewZoom))
        builderState.previewZoom = Math.max(
          0.5,
          Math.min(savedUi.previewZoom, 1.5),
        );
      if (typeof savedUi?.fitWidth === "boolean")
        builderState.fitWidth = savedUi.fitWidth;
    } catch (_) {}

    const defaultRun = (text, bold = false) => ({
      text,
      bold,
      italic: false,
      underline: false,
    ...
    const blockDefaults = (type) =>
      ({
        heading: {
          ...
          runs: [defaultRun("Nowy nagłówek", true)],
        },
        paragraph: {
          type: "paragraph",
          format: { alignment: "left" },
          content: "Nowy akapit",
          runs: [defaultRun("Nowy akapit")],
        },
        agreement_section: {
        ...
        ordered_list: {
          ...
          items: [
            {
              content: "Pierwszy punkt",
              runs: [defaultRun("Pierwszy punkt")],
              level: 0,
            },
          ],
          list_styles: defaultListStyles(),
        },
        bullet_list: {
          type: "bullet_list",
          items: [
        ...
        form_field: {
          ...
          field:
            variables.find(
              (item) =>
                item.category === "Pola formularza" &&
                !item.name.endsWith("_display") &&
                !item.name.endsWith("_yes_no"),
            )?.name || "",
        ...
        statement: {
          ...
          runs: [defaultRun("Treść oświadczenia")],
        },
        participant_signature: {
          type: "participant_signature",
          label: "Czytelny podpis uczestnika",
        },
        criteria_table: {
          type: "criteria_table",
          criteria: [],
    ...
    function normalizeListStyles(value) {
      ...
      return defaults.map((fallback, level) => {
        ...
        const marker = listMarkerLabels.some(([name]) => name === raw.marker)
          ? raw.marker
          : fallback.marker;
        ...
        return {
          level,
          marker,
          indent_mm: Number.isFinite(requestedIndent)
            ? Math.max(0, Math.min(requestedIndent, 60))
            : fallback.indent_mm,
        };
      });
    }
    ...
    function textBlockFormat(block) {
      const textBlock = ["heading", "paragraph", "statement"].includes(
        block?.type,
      );
      if (!textBlock) return null;
      const raw =
        block.format && typeof block.format === "object" ? block.format : {};
      const alignment = ["left", "center", "right", "justify"].includes(
        raw.alignment,
      )
        ? raw.alignment
        : ["left", "center", "right", "justify"].includes(block.alignment)
          ? block.alignment
          : "left";
      return {
        bold: Object.prototype.hasOwnProperty.call(raw, "bold")
          ? raw.bold === true
          : block.type === "heading",
    ...
    function normalizeRuns(value, fallbackText = "", fallbackStyle = {}) {
      const source = Array.isArray(value)
        ? value
        : [{ text: fallbackText, ...fallbackStyle }];
      const result = [];
      source.forEach((item) => {
    ...
    function legacyHtmlToRuns(value, fallbackStyle = {}) {
      ...
      const walk = (node, style) => {
        if (node.nodeType === Node.TEXT_NODE) {
          normalizeRuns([{ text: node.nodeValue || "", ...style }]).forEach(
            (run) => {
        ...
        const next = {
          ...
          underline:
            style.underline ||
            tag === "u" ||
            node.classList.contains("document-inline-underline") ||
            node.classList.contains("document-text-underline"),
        };
        node.childNodes.forEach((child) => walk(child, next));
      };
      template.content.childNodes.forEach((node) =>
        walk(node, {
          bold: fallbackStyle.bold === true,
          italic: fallbackStyle.italic === true,
          underline: fallbackStyle.underline === true,
        }),
      );
    ...
    function normalizeDocumentModel(value) {
      ...
      model.blocks.forEach((block) => {
        ...
        if (["ordered_list", "bullet_list"].includes(block.type)) {
          let previousLevel = 0;
          block.items = (Array.isArray(block.items) ? block.items : []).map(
            (rawItem, itemIndex) => {
              const item =
                rawItem && typeof rawItem === "object"
                  ? rawItem
                  : { content: String(rawItem ?? "") };
              normalizeHolder(item);
              const requestedLevel = Math.max(
                0,
                Math.min(Number.parseInt(item.level, 10) || 0, 3),
              );
              item.level =
                itemIndex === 0
                  ? 0
                  : Math.min(requestedLevel, previousLevel + 1);
        ...
        if (block.type === "table") {
          block.rows = (Array.isArray(block.rows) ? block.rows : []).map(
            (row) =>
        ...
        if (block.type === "criteria_table") {
          const used = new Set();
          block.criteria = (
            Array.isArray(block.criteria) ? block.criteria : []
          ).flatMap((item) => {
    ...
    function holderHasText(holder) {
      ...
      if (Array.isArray(holder.runs))
        return holder.runs.some((run) => String(run?.text || "").trim());
      return Boolean(String(holder.content || "").trim());
    }

    function blockHasRealContent(block) {
      if (!block || typeof block !== "object") return false;
      if (["heading", "paragraph", "statement"].includes(block.type))
        return holderHasText(block);
      if (block.type === "agreement_section")
        return Boolean(String(block.number || block.title || "").trim());
      if (["ordered_list", "bullet_list"].includes(block.type))
        return Array.isArray(block.items) && block.items.some(holderHasText);
      if (block.type === "table")
        return (
          ...
          block.rows.some(
            (row) =>
              ...
              row.some((cell) =>
                typeof cell === "object"
                  ? holderHasText(cell)
                  : Boolean(String(cell || "").trim()),
              ),
          )
        );
      if (block.type === "training_table")
        return Array.isArray(block.columns) && block.columns.length > 0;
      if (
        ["participant_data", "participant_contact", "project_info"].includes(
          block.type,
        )
      )
      ...
      if (block.type === "signatures")
        return Boolean(
          String(block.left_label || block.right_label || "").trim(),
        );
      if (block.type === "form_field")
        return Boolean(String(block.field || "").trim());
      if (block.type === "criteria_table")
        return Array.isArray(block.criteria) && block.criteria.length > 0;
      if (block.type === "participant_signature")
        return Boolean(String(block.label || "").trim());
      return block.type === "participant_address";
    }

    function documentHasRealContent(value) {
      if (!value || !Array.isArray(value.blocks) || !value.blocks.length)
        return false;
      const containsEmptyParagraphRuns = value.blocks.some(
        (block) =>
      ...
      return (
        !containsEmptyParagraphRuns && value.blocks.some(blockHasRealContent)
      );
    }
    ...
    function validatedDraftEnvelope(value, { legacy = false } = {}) {
      ...
      const envelope = legacy
        ...
        : value;
      if (Number(envelope.schemaVersion) !== BUILDER_SCHEMA_VERSION)
        return null;
      if (
        Number(envelope.formId) !== formId ||
        envelope.documentType !== documentType
      )
        return null;
      if (timestampValue(envelope.savedAt) === null) return null;
      if (Number(envelope.document?.version) !== BUILDER_SCHEMA_VERSION)
        return null;
      if (
        envelope.document?.document_type &&
        envelope.document.document_type !== documentType
      )
        return null;
    ...
    function readDraft(key, options = {}) {
      ...
      try {
        ...
        debugEvent("local draft ignored", {
          format: options.legacy ? "legacy" : "current",
          reason: "invalid-json",
        });
        storageRemove(key);
        return null;
    ...
    function loadLocalDraft() {
      ...
      if (legacyLocalKey && legacyLocalKey !== localKey) {
        const legacyDraft = draft
          ? null
          : readDraft(legacyLocalKey, { legacy: true });
      ...
      const isNewer = backendIsNew
        ? true
        : backendTime !== null && savedAt > backendTime;
      if (!isNewer) {
        debugEvent("local draft ignored", {
          format: "current",
          reason: "not-newer",
    ...
    function initialBackendStatus() {
      return backendIsNew
        ? "Utworzono nowy szablon."
        : "Wczytano zapisany szablon.";
    }
    ...
    function blockFormatClasses(block) {
      ...
      return [`is-align-${formatting.alignment}`].filter(Boolean).join(" ");
    }
    ...
    function conditionEditor(block, index) {
      ...
      const options = [
        ...
        ...variables
          ...
          .map(
            (item) =>
              `<option value="${escapeHtml(item.name)}" ${condition.variable === item.name ? "selected" : ""}>${escapeHtml(item.label)}</option>`,
          ),
      ].join("");
  ...
  return normalizeRuns(holder?.runs, holder?.content || "")
    ...
    function orderedListMarkers(block) {
      ...
      return (block.items || []).map((item) => {
        ...
        if (marker === "lower-roman-dot")
          return `${romanNumber(counters[level]).toLowerCase()}.`;
        if (marker === "upper-roman-dot")
          return `${romanNumber(counters[level])}.`;
    ...
    function renderListBlock(block, index) {
      ...
      const styles = ordered
        ? normalizeListStyles(block.list_styles)
        : defaultListStyles();
      const markers = ordered
        ? orderedListMarkers(block)
        : (block.items || []).map(() => "•");
      const items = (block.items || [])
        ...
        .join("");
      const configuration = ordered
        ? `
    <details class="agreement-builder-list-config-collapse">
        <summary>
            <span>Numeracja i wcięcia poziomów</span>
            <span class="agreement-builder-list-config__chevron" aria-hidden="true">▼</span>
        </summary>

        <div class="agreement-builder-list-config">
            ${styles
              .map(
                (style) => `
                <div>
                    <strong>Poziom ${style.level + 1}</strong>

                    <label>
                        Marker
                        <select
                            data-builder-list-marker
                            data-index="${index}"
                    ...
                    <label>
                        ...
                        <input
                            ...
                            max="60"
                            step="1"
                            value="${style.indent_mm}"
                            data-builder-list-indent
                            data-index="${index}"
                            data-level="${style.level}"
                        >
                    </label>
...
`
    ...
    function renderCriteriaTableBlock(block, index) {
      const catalog = new Map(
        criteriaVariables.map((item) => [item.field_key, item]),
      );
      const selected = (block.criteria || [])
        .map((item, criterionIndex) => {
          const criterion = catalog.get(item.field_key);
          const label =
            criterion?.label || `Nieaktywne kryterium: ${item.field_key}`;
      ...
      const selectedKeys = new Set(
        (block.criteria || []).map((item) => item.field_key),
      );
      const available = criteriaVariables.filter(
        (item) => !selectedKeys.has(item.field_key),
      );
      const options = available.length
        ? available
            .map(
              (item) =>
                `<option value="${escapeHtml(item.field_key)}">${escapeHtml(item.label)}</option>`,
            )
            .join("")
    ...
    function renderBlock(block, index) {
      ...
      if (block.type === "heading") {
        ...
        const choices = [
          ["participant_name", "Imię i nazwisko"],
          ["pesel", "PESEL"],
          ["participant_address", "Adres"],
          ["telefon", "Telefon"],
          ["email", "E-mail"],
        ...
        const selectedField = fieldVariables.find(
          (item) => item.name === block.field,
        );
        const booleanField = ["checkbox", "boolean", "bool"].includes(
          String(selectedField?.type || "").toLowerCase(),
        );
        ...
        body =
          '<p class="workflow-help">Gotowy adres uczestnika z pól formularza.</p>';
        ...
        const choices = [
          ...
          ["project_action", "Działanie"],
          ["funding_source", "Źródło finansowania"],
          ["institution_name", "Instytucja"],
        ];
        ...
        body =
          '<div class="agreement-builder-block__page-break">Podział strony</div>';
      }
    ...
    function holderForEditable(target) {
      ...
      if (target.matches("[data-builder-list-item]"))
        return block.items?.[Number(target.dataset.itemIndex)] || null;
      if (target.matches("[data-builder-table-cell]"))
        return (
          block.rows?.[Number(target.dataset.rowIndex)]?.[
            Number(target.dataset.cellIndex)
          ] || null
        );
      if (
        target.dataset.builderField === "content" &&
        ["heading", "paragraph", "statement"].includes(block.type)
      )
        return block;
      return null;
    }
      ...
      const walk = (node, style) => {
        ...
        const next = {
          bold:
            ...
            node.dataset.bold === "true",
          italic:
            style.italic ||
            ["em", "i"].includes(tag) ||
            node.dataset.italic === "true",
          underline:
            style.underline || tag === "u" || node.dataset.underline === "true",
        };
        node.childNodes.forEach((child) => walk(child, next));
        if (["div", "p"].includes(tag) && node !== target && node.nextSibling)
          append("\n", next);
      };
      target.childNodes.forEach((node) =>
        walk(node, { bold: false, italic: false, underline: false }),
      );
    ...
    function syncEditableToModel(target) {
      ...
      const block = documentModel.blocks[Number(target.dataset.index)];
      if (block && target.dataset.builderField)
        block[target.dataset.builderField] = target.textContent || "";
    }
    ...
    function editorForRange(range) {
      ...
      const target = container?.closest?.(
        '[contenteditable="true"][data-builder-editor-key]',
      );
    ...
    function editorContainsRange(range, target = activeEditable) {
      return Boolean(
        target &&
        blocksNode.contains(target) &&
        editorForRange(range) === target,
      );
    }
    ...
    function selectionOffsets(target, range) {
      ...
      return {
        key: target.dataset.builderEditorKey,
    ...
    function restoreTextSelection() {
      if (savedTextSelection && editorContainsRange(savedTextSelection)) {
        try {
          ...
          activeSelection =
            selectionOffsets(activeEditable, savedTextSelection) ||
            activeSelection;
          return true;
        } catch (_) {
          savedTextSelection = null;
    ...
    function restoreEditableSelection(saved) {
      const target = blocksNode.querySelector(
        `[data-builder-editor-key="${CSS.escape(saved.key || "")}"]`,
      );
      if (!target) {
        activeSelection = null;
        savedTextSelection = null;
    ...
    function readField(target) {
      ...
      if (!block) return;
      if (
        target.matches("[data-builder-list-marker], [data-builder-list-indent]")
      ) {
        block.list_styles = normalizeListStyles(block.list_styles);
        const level = Math.max(
          0,
          Math.min(Number(target.dataset.level) || 0, 3),
        );
        if (target.matches("[data-builder-list-marker]"))
          block.list_styles[level].marker = target.value;
        else
          block.list_styles[level].indent_mm = Math.max(
            0,
            Math.min(Number(target.value) || 0, 60),
          );
      }
      ...
      if (field) {
        ...
        if (field === "field" && block.display === "yes_no") {
          ...
          if (
            !["checkbox", "boolean", "bool"].includes(
      ...
      if (target.dataset.builderArray) {
        block[target.dataset.builderArray] = [
          ...root.querySelectorAll(
            `[data-builder-array="${target.dataset.builderArray}"][data-index="${index}"]:checked`,
          ),
        ].map((item) => item.value);
      }
      if (target.matches("[data-builder-list-item], [data-builder-table-cell]"))
      ...
      if (target.matches("[data-builder-condition-variable]")) {
        if (target.value)
          block.condition = {
            ...
            operator: block.condition?.operator || "not_empty",
          };
        else delete block.condition;
        render();
      }
      if (target.matches("[data-builder-condition-operator]")) {
        block.condition = {
          ...(block.condition || {}),
      ...
      if (target.matches("[data-builder-condition-value]"))
        block.condition = { ...(block.condition || {}), value: target.value };
      changed();
    }
    ...
    function refreshControls() {
      root
        .querySelector('[data-builder-action="undo"]')
        ?.toggleAttribute("disabled", historyIndex <= 0);
      root
        .querySelector('[data-builder-action="redo"]')
    ...
    function setActiveBlock(index) {
      activeBlockIndex =
        Number.isInteger(index) && documentModel.blocks[index] ? index : -1;
    ...
    function activeListItem() {
      ...
      if (
        !["ordered_list", "bullet_list"].includes(block?.type) ||
    ...
    function changeListItemLevel(direction) {
      ...
      const previous =
        active.itemIndex > 0
          ? Number(active.block.items[active.itemIndex - 1].level) || 0
          : -1;
    ...
    function addCriterion(blockIndex) {
      const select = blocksNode.querySelector(
        `[data-builder-criterion-select][data-index="${blockIndex}"]`,
      );
      ...
      if (!block.criteria.some((item) => item.field_key === fieldKey))
        block.criteria.push({ field_key: fieldKey });
      render();
      changed({ checkpointNow: true });
    }
    ...
    function moveCriterion(blockIndex, criterionIndex, direction) {
      ...
      [block.criteria[criterionIndex], block.criteria[targetIndex]] = [
        block.criteria[targetIndex],
        block.criteria[criterionIndex],
      ];
      render();
      changed({ checkpointNow: true });
    }
    ...
    function refreshFormattingControls() {
      ...
      const formatting = textBlockFormat(block);
      const target =
        activeEditable && document.contains(activeEditable)
          ? activeEditable
          : null;
      const holder = holderForEditable(target);
      const saved = target
        ? editableSelection(target) || activeSelection
        : activeSelection;
      const inlineState = holder
        ? inlineFormattingState(holder.runs, saved)
        : null;
      ...
      root.querySelectorAll("[data-builder-list-level]").forEach((button) => {
        ...
        const previous =
          activeList && activeList.itemIndex > 0
            ? Number(activeList.block.items[activeList.itemIndex - 1].level) ||
              0
            : -1;
        const maximum =
          activeList?.itemIndex > 0 ? Math.min(3, previous + 1) : 0;
        button.disabled =
          !activeList ||
          (direction === "decrease" ? current <= 0 : current >= maximum);
      });
    }
    ...
    function inlineFormattingState(runs, saved) {
      ...
      return Object.fromEntries(
        ["bold", "italic", "underline"].map((name) => [
          name,
          relevant.every((run) => run[name] === true),
        ]),
      );
    }
    ...
    function applyInlineFormat(name) {
      if (
        !["bold", "italic", "underline"].includes(name) ||
      ...
      runs.forEach((run) => {
        ...
        if (runStart < cutStart)
          result.push({ ...run, text: run.text.slice(0, cutStart - runStart) });
        if (cutStart < cutEnd)
          result.push({
            ...run,
            [name]: nextValue,
            text: run.text.slice(cutStart - runStart, cutEnd - runStart),
          });
        if (cutEnd < runEnd)
          result.push({ ...run, text: run.text.slice(cutEnd - runStart) });
        cursor = runEnd;
      });
    ...
    function alignBlock(alignment) {
      const block = activeTextBlock();
      if (!block || !["left", "center", "right", "justify"].includes(alignment))
        return;
    ...
    function selectedParams(formData) {
      ...
      if (previewTraining?.value)
        formData.set("training_id", previewTraining.value);
    }
    ...
    function previewErrorMessage(payload) {
      const missing = Array.isArray(payload?.missing_variables)
        ? payload.missing_variables.filter(Boolean)
        : [];
    ...
    async function preview({ resetScroll = false, force = false } = {}) {
      if (root.hidden) {
        setPreviewState(
          "waiting",
          "Podgląd uruchomi się po otwarciu kreatora.",
        );
        return;
      }
      ...
      try {
        ...
        if (!response.ok || !payload.ok) {
          debugEvent("preview request failed", {
            requestGeneration,
            status: response.status,
          });
          if (response.status === 422)
            blockedPreviewFingerprint = previewFingerprint;
        ...
        debugEvent("preview request success", {
          requestGeneration,
          status: response.status,
        });
      } catch (error) {
        ...
        if (previewState.dataset.previewState === "loading")
    ...
    async function save(action) {
      ...
      statusNode.textContent =
        action === "activate" ? "Aktywowanie…" : "Zapisywanie…";
      ...
      window.opener?.postMessage(
        {
          type:
            documentType === "agreement"
              ? "agreement-builder-saved"
              : "document-builder-saved",
          formId,
          documentType,
      ...
      if (action === "activate") {
        const radio = document.querySelector(
          `input[name="${documentType === "agreement" ? "contract" : "declaration"}_template_source"][value="builder"]`,
        );
    ...
    function persistBuilderState() {
      try {
        localStorage.setItem(uiStorageKey, JSON.stringify(builderState));
      } catch (_) {}
    }
    ...
    function applyBuilderLayout({ persist = true, refit = true } = {}) {
      ...
      root.dataset.previewZoom = String(
        Math.round(builderState.previewZoom * 100),
      );
      ...
      variablesPane.setAttribute(
        "aria-hidden",
        String(!builderState.variablesVisible),
      );
      ...
      root.querySelectorAll("[data-builder-size-mode]").forEach((button) => {
        const active =
          button.dataset.builderSizeMode === builderState.workspaceSizeMode;
        button.classList.toggle("is-active", active);
        button.setAttribute("aria-pressed", String(active));
      });
      root
        .querySelectorAll("[data-builder-variables-toggle]")
        .forEach((button) => {
      ...
      if (output)
        output.textContent = `${Math.round(builderState.previewZoom * 100)}%`;
      if (persist) persistBuilderState();
      if (builderState.fitWidth && previewVisible && refit)
        window.requestAnimationFrame(() => fitPreview({ persist }));
    }

    function setZoom(value, { fitWidth = false, persist = true } = {}) {
      builderState.previewZoom = Math.max(
        0.5,
        Math.min(Number(value) || 0.8, 1.5),
      );
    ...
    function fitPreview({ persist = true } = {}) {
      ...
      const pageWidth =
        renderedWidth > 0
          ? renderedWidth / builderState.previewZoom
          : (210 / 25.4) * 96;
    ...
    async function downloadPdf() {
      ...
      if (!response.ok || contentType.includes("application/json")) {
        showErrors(
          ...
          "Nie można wygenerować przykładowego PDF.",
        );
        return;
      }
      ...
      link.download =
        documentType === "agreement"
          ? "przykladowa-umowa.pdf"
          : "przykladowa-deklaracja.pdf";
      link.click();
    ...
    function insertVariable(name) {
      ...
      let target =
        activeEditable && document.contains(activeEditable)
          ? activeEditable
          : blocksNode.querySelector("[contenteditable=true]");
      if (!target) {
        ...
        render();
        target = blocksNode.querySelector(
          "[data-builder-block]:last-child [contenteditable=true]",
        );
      }
      activeEditable = target;
      const holder = holderForEditable(target);
      if (!holder) return;
      const saved = editableSelection(target) ||
        activeSelection || {
          key: target.dataset.builderEditorKey,
          start: holder.content?.length || 0,
          end: holder.content?.length || 0,
        };
      syncEditableToModel(target);
      const token = `{{ ${name} }}`;
      const runs = normalizeRuns(holder.runs);
      ...
      runs.forEach((run) => {
        ...
        if (runStart < start)
          result.push({
            ...
            text: run.text.slice(
              0,
              Math.min(run.text.length, start - runStart),
            ),
          });
        if (!inserted && runEnd >= start) {
          result.push({ text: token, ...styleSource });
          inserted = true;
        }
        if (runEnd > end)
          result.push({
            ...run,
            text: run.text.slice(Math.max(0, end - runStart)),
          });
        cursor = runEnd;
      });
    ...
    function runAtOffset(runs, offset) {
      ...
      return {
        ...
        italic: Boolean(last?.italic),
        underline: Boolean(last?.underline),
      };
    }
    ...
    function renderRecent(explicit) {
      ...
      if (!recent)
        try {
          recent = JSON.parse(
            localStorage.getItem(`${localKey}-recent-variables`) || "[]",
          );
        } catch (_) {
          recent = [];
        }
      ...
      list.innerHTML = valid
        .map(
          (name) =>
            `<button type="button" class="builder-button builder-button--secondary builder-button--compact" data-builder-insert-variable="${escapeHtml(name)}">{{ ${escapeHtml(name)} }}</button>`,
        )
        .join("");
    }
    ...
    root.addEventListener("keydown", (event) => {
      if (
        event.key !== "Tab" ||
        !event.target.matches?.("[data-builder-list-item]")
      )
        return;
      event.preventDefault();
    ...
    root.addEventListener("click", async (event) => {
      if (
        event.target.closest("[data-builder-draft-restore]") &&
        pendingLocalDraft
      ) {
      ...
      if (selectedArticle)
        setActiveBlock(Number(selectedArticle.dataset.index));
      const add = event.target.closest("[data-builder-add]");
      if (add) {
        documentModel.blocks.push(clone(blockDefaults(add.dataset.builderAdd)));
        activeBlockIndex = documentModel.blocks.length - 1;
        render();
        changed();
        return;
      ...
      if (remove) {
        const removed = Number(remove.dataset.index);
        documentModel.blocks.splice(removed, 1);
        activeBlockIndex = Math.min(removed, documentModel.blocks.length - 1);
        render();
        changed();
        return;
      }
      ...
      if (move) {
        ...
        const to = move.dataset.builderMove === "up" ? from - 1 : from + 1;
        if (to >= 0 && to < documentModel.blocks.length) {
          [documentModel.blocks[from], documentModel.blocks[to]] = [
            documentModel.blocks[to],
            documentModel.blocks[from],
          ];
          activeBlockIndex = to;
          render();
          changed();
      ...
      if (listLevel) {
        changeListItemLevel(listLevel.dataset.builderListLevel);
        return;
      }
      const addItem = event.target.closest("[data-builder-add-item]");
      if (addItem) {
        const block = documentModel.blocks[Number(addItem.dataset.index)];
        const level = Number(block.items.at(-1)?.level) || 0;
        block.items.push({
          content: "Nowy punkt",
          runs: [defaultRun("Nowy punkt")],
          level,
        });
        render();
        changed();
      ...
      if (removeItem) {
        documentModel.blocks[Number(removeItem.dataset.index)].items.splice(
          Number(removeItem.dataset.itemIndex),
          1,
        );
        render();
        changed();
      ...
      const addCriterionButton = event.target.closest(
        "[data-builder-add-criterion]",
      );
      if (addCriterionButton) {
        addCriterion(Number(addCriterionButton.dataset.index));
        return;
      }
      const removeCriterion = event.target.closest(
        "[data-builder-remove-criterion]",
      );
      if (removeCriterion) {
        const block =
          documentModel.blocks[Number(removeCriterion.dataset.index)];
        block.criteria.splice(
          Number(removeCriterion.dataset.criterionIndex),
          1,
        );
        render();
        changed({ checkpointNow: true });
      ...
      const moveCriterionButton = event.target.closest(
        "[data-builder-move-criterion]",
      );
      if (moveCriterionButton) {
        moveCriterion(
          Number(moveCriterionButton.dataset.index),
          Number(moveCriterionButton.dataset.criterionIndex),
          moveCriterionButton.dataset.builderMoveCriterion,
        );
        return;
      }
      const addRow = event.target.closest("[data-builder-add-row]");
      if (addRow) {
        const table = documentModel.blocks[Number(addRow.dataset.index)];
        table.rows.push(Array(table.rows[0]?.length || 2).fill(""));
        render();
        changed();
        return;
      }
      ...
      if (variable) {
        insertVariable(variable.dataset.builderInsertVariable);
        return;
      }
      const copy = event.target.closest("[data-builder-copy-variable]");
      if (copy) {
        await navigator.clipboard.writeText(
          `{{ ${copy.dataset.builderCopyVariable} }}`,
        );
        if (!builderState.dirty) statusNode.textContent = "Zmienna skopiowana";
        return;
      }
      const format = event.target.closest("[data-builder-format]");
      if (format) {
        applyInlineFormat(format.dataset.builderFormat);
        return;
      }
      const align = event.target.closest("[data-builder-align]");
      if (align) {
        alignBlock(align.dataset.builderAlign);
        return;
      }
      const action = event.target.closest("[data-builder-action]")?.dataset
        .builderAction;
      ...
      if (event.target.closest("[data-builder-preview-now]"))
        await preview({ force: true });
      if (event.target.closest("[data-builder-download-pdf]"))
        await downloadPdf();
      const errorPath = event.target.closest("[data-builder-error-path]")
        ?.dataset.builderErrorPath;
      if (errorPath?.startsWith("blocks."))
        blocksNode
          .querySelector(`[data-index="${Number(errorPath.split(".")[1])}"]`)
      ...
      if (size) {
        ...
        builderState.workspaceSizeMode =
          builderState.workspaceSizeMode === requestedMode
            ? "balanced"
            : requestedMode;
        applyBuilderLayout();
      }
      ...
      const zoom = event.target.closest("[data-builder-zoom]")?.dataset
        .builderZoom;
      ...
      if (event.target.closest("[data-builder-close-cancel]"))
        closeDialog?.close();
      if (event.target.closest("[data-builder-close-discard]")) {
        builderState.dirty = false;
        closeDialog?.close();
        window.close();
      }
      if (event.target.closest("[data-builder-close-save]")) {
        if (await save("draft")) {
          closeDialog?.close();
          window.close();
        }
    ...
    root.addEventListener("mousedown", (event) => {
      if (
        event.target.closest(
          "[data-builder-align], [data-builder-insert-variable], [data-builder-list-level]",
        )
      )
        event.preventDefault();
    });
    root.addEventListener("dragstart", (event) => {
      ...
      if (variable)
        event.dataTransfer.setData(
          "text/agreement-variable",
          variable.dataset.builderVariable,
        );
      const block = event.target.closest("[data-builder-block]");
      if (block && !variable)
        event.dataTransfer.setData("text/agreement-block", block.dataset.index);
    });
    blocksNode.addEventListener("dragover", (event) => event.preventDefault());
    blocksNode.addEventListener("drop", (event) => {
      event.preventDefault();
      const variableName = event.dataTransfer.getData(
        "text/agreement-variable",
      );
      if (variableName) {
        const editableTarget = event.target.closest("[contenteditable=true]");
        if (editableTarget) activeEditable = editableTarget;
      ...
      if (Number.isInteger(from) && target) {
        const [block] = documentModel.blocks.splice(from, 1);
        activeBlockIndex = Number(target.dataset.index);
        documentModel.blocks.splice(activeBlockIndex, 0, block);
        render();
        changed();
      }
    });
    ...
    root
      .querySelector("[data-builder-variable-search]")
      ?.addEventListener("input", (event) => {
        const query = event.target.value.trim().toLocaleLowerCase("pl");
        root.querySelectorAll("[data-builder-variable]").forEach((item) => {
          item.hidden =
            Boolean(query) &&
            !item.dataset.builderVariableSearchText.includes(query);
        });
        root
          .querySelectorAll("[data-builder-variable-category]")
          .forEach((category) => {
            category.hidden = !category.querySelector(
              "[data-builder-variable]:not([hidden])",
            );
          });
      });
    previewData?.addEventListener("change", () => {
      const submission = submissions.find(
        (item) => item.submission_id === previewData.value,
      );
      const trainings = submission?.trainings || [];
      if (previewTraining) {
        previewTraining.replaceChildren(
          new Option("Wybierz szkolenie", ""),
          ...trainings.map((item) => new Option(item.label, item.id)),
        );
    ...
    previewTraining?.addEventListener("change", () =>
      schedulePreview({ resetScroll: true }),
    );
    ...
    if (typeof ResizeObserver === "function" && previewViewport) {
      new ResizeObserver(() => {
        if (builderState.fitWidth && !previewPane.hidden)
          fitPreview({ persist: false });
    ...
    function initializeDocumentBuilder() {
      ...
      resetHistory();
      renderRecent();
      if (mobileQuery.matches && builderState.viewMode === "split")
        builderState.viewMode = "editor";
      applyBuilderLayout({ persist: false });
      schedulePreview();
      debugEvent("initialized", {
        source: pendingLocalDraft
          ? "backend-with-newer-local-draft"
          : backendIsNew
            ? "new"
            : "backend",
        blockCount: documentModel.blocks.length,
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document
      .querySelectorAll("[data-document-builder], [data-agreement-builder]")
      .forEach(initializeBuilder);
  });
})();
