...
Po wgraniu i poprawnej weryfikacji podpisu beneficjenta zgłoszenie otrzymuje status `AGREEMENT_WAITING_FOR_OFFICE_SIGNATURE`. Dopiero potwierdzenie podpisu urzędu rejestruje `AGREEMENT_SIGNED_BY_OFFICE`, po czym proces może przejść do `PROCESS_COMPLETED`. Kody `AGREEMENT_WAITING_FOR_SIGNATURE`, `AGREEMENT_UPLOADED`, `AGREEMENT_SIGNED` i `BENEFICIARY_AGREEMENT_*` są obsługiwane wyłącznie dla danych historycznych.
